...
def test_update_datasets(
    dataset_container: DataContainer,
    initial_datasets: dict[str, str | None],
    updated_datasets: list[tuple[str, str]],
    path: str,
    request: pytest.FixtureRequest,
):
    # Request initial test data from the fixtures
...
def test_update_datasets_non_existing(
    dataset_container: DataContainer,
    initial_datasets: dict[str, str],
    updated_datasets: list[tuple[str, str]],
    expected_error: type[Exception],
    request: pytest.FixtureRequest,
):
    # Add initial dataset to root
    initial_test_data = {key: request.getfixturevalue(value) for key, value in initial_datasets.items()}
    dataset_container.add_datasets("/", **initial_test_data)

    # Try to update with invalid paths
    update_data = [(path, request.getfixturevalue(fixture)) for path, fixture in updated_datasets]
